...
description: "Hello GPU 第13章 · 综合逐元素、归约与融合，独立完成一次可复现的 Kernel 优化闭环"
---
...
`code/part2-kernels/chapter13/rmsnorm_hip.hip` 的 `serial` 版本让一个 GPU 线程处理一整行：先顺序累加平方，再顺序写回归一化结果。
