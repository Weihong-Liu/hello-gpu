...
![AUP Learning Cloud 登录页面](./images/01-login-page.png)

### 3. 选择登录方式

平台提供两种登录方式：**GitHub 账户登录（推荐）** 和 **本地账户登录**（本地账户需管理员提供）。

![选择登录方式](./images/02-login-methods.png)
...
![GitHub 账户授权登录](./images/03-github-login.png)
...
![可用资源目录](./images/06-available-resources-1.png)

![可用资源目录（资源类别）](./images/07-available-resources-2.png)
...
![Code Server 启动配置页面](./images/09-code-server-launch.png)
...
![端口转发通知](./images/10-port-forward-1.png)

3. 也可以在底部 **PORTS** 面板中查看所有已转发的端口

![PORTS 端口面板](./images/11-port-forward-2.png)
...
![端口转发地址示例](./images/12-port-forward-url.png)
...
![安装新插件](./images/13-install-extension.png)
...
![File → Hub Control Panel 退出入口](./images/14-exit-hub-control-panel.png)
...
![Code Server 左下角 JupyterHub 按钮回到主页](./images/16-code-server-back-home.png)
...
![Stop my server 关闭当前镜像](./images/15-stop-my-server.png)
