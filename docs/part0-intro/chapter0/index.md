...
选择 9070XT（RDNA4）有两个很实际的理由：

| 理由 | 说明 |
| ---- | ---- |
| 易获得、成本低 | 9070XT 是消费级显卡，不像数据中心卡那样昂贵和稀缺。大多数学习者都能弄到手 |
...
最后再强调一次：**本书所有实验数据基于 Radeon RX 9070 XT + ROCm 7.13 + 原生 Ubuntu 24.04**。其他 AMD GPU 可以参考方法论，但性能数字和工具可用性需要自己实测验证。
...
市面上很多教程把 Vector Add / Reduction / Softmax / Matmul 当成"知识点"逐个讲一遍，讲完就结束。我们的做法不一样：**每个算子都呈现完整的优化路径**——从最 naive 的版本开始，一步步 profiling、定位瓶颈、改写、再测，直到接近硬件极限。这和你在 LeetGPU 这类平台上"刷题"的过程是一致的：先正确性、再带宽、最后计算强度。学完整本书，你应该能上手任何 GPU 算子题库。
...
下一章会带你做环境准备。它不会一上来就展开 ROCm 软件栈的内部原理，只帮你确认几件最关键的事：ROCm 能不能看到 9070XT？PyTorch 能不能跑？最小 HIP 程序能不能编译运行？几道门推开，后面的路就是通的。
...
- 实验结论基于 **Radeon RX 9070 XT + ROCm 7.13 + 原生 Ubuntu 24.04**，其他设备的读者请以方法论为主、自行复测数据。
- 学习路线从环境验证起步，经过 profiling、算子优化、刷题，最终落到 Agent 自动化——每一站都是下一站的基础。
- 下一章解决第一个动手问题：**怎样确认你的 9070XT 实验环境可以继续往后跑？**
